...
const Referral: React.FC<counterInterface> = ({target,setCompToDisplay}) => {
    ...
    const timeTaken = todayRow?.referrals_time_in_mm;
    ...
    function increase(){
        ...
        if (todayRow) {
            const updatedRow = {
                ...todayRow,
                referrals: (todayRow.referrals || 0) + 1,
                referrals_time_in_mm:parseFloat(((timeTaken || 0) + ((Date.now() - mountTime) / 60000)).toFixed(2)),
            };
...
function decrease(){
    if(todayRow && todayRow.referrals !== 0){
        setCurrentCount(currentCount-1);
        if (todayRow) {
            const updatedRow = {
                ...todayRow,
    ...
    return (
            <IonContent className="ion-padding">
                <div className='flex flex-col justify-center items-center'>
                    ...
                    <span className='font-bold'>Time:{todayRow?.referrals_time_in_mm} Minutes</span>
                    <IonCard className='h-60 w-60 p-4 flex items-center justify-center'>
                        <div className='h-40 w-40 bg-slate-300 rounded-full relative flex items-center justify-center'>
                            <span className='font-extrabold text-6xl'>{todayRow?.referrals}</span>
